...
void			exec_pipe(t_cmd *curr, t_minishell *minishell)
{
	...
	waitpid(pipe_fd[0], &cond2, WCONTINUED); //WNOHANG : 종료되지 않더라도 리턴하라.

	if ((p2 = fork()) < 0)
		exit(1258) ; //id가 
	//그게 아니고, p2가 자식프로세스라면 -
	ft_printf("p2이 몇이죠? %d ", p2);
	if (p2 == 0)
	{
		close(pipe_fd[1]);
		...
		waitpid(pipe_fd[1], &cond1, WUNTRACED); //중단되었다가 재개된 자식프로세스의 상태를 받음.
